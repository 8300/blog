...
 
  
用于在真正内核装载前，做一些操作（俗称两阶段启动），比如加载module ，mount 一些非root 分区，提供灾难恢复shell 环境等， 资料 ，我是期望直接从kernel image 直接启动，所以没选它 .详情见《Linux内核启动流程》
 
...
：支持事件轮循的系统调用。 
 
：除非你在很少的内存且不使用交换内存时，才不要选择这项。后面的这四项都是在编译时内存中的对齐方式，0 表示 
...
维护一个devtmpfs文件系统以便在/dev上安装，devtmpfs 的功用是在 Linux 核心 启动早期建立一个初步的 /dev，令一般启动程序不用等待 udev，缩短 GNU/Linux 的开机时间。
...
用不到人力工程学设备 
 
 <=== 这个选项，对于跑物理机建议 开启，因为有可能你的键盘是USB 的，我是跑虚拟机的，所以关了  
 

 
   <=== 发光二级管，应该是跟显示器相关的驱动，由于我运行 的是虚拟机，所以我选择关闭  
 
  
 
 <=== 一些笔记本的驱动，我没有相关设备 
...
1、 make clean   
这条命令是在正式编译你的内核之前先把环境给清理干净。有时你也可以用 make realclean或 make mrproper 来彻底清除相关依赖，保证没有不正确的.o 文件存在。  
2 、make dep  
这条命令是编译相关依赖文件。  
3 、make zImage  
...
 
PS: 
其中标红的部分为我认为可以不编入内核的，标绿的为我不确定可否不编入内核的，标蓝的我不明白的。 
Networking support和Device Drivers两部分内容实在太多了，我没有一一的看，找了网上一些人的对两部分的配置及看法。
...
cp arch/arm/boot/uImage ~/tftp_boot/3531d/uImage_v600
